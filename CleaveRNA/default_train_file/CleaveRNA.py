...
def train(args):
    ...
    # Check if 'generated_merged_num.csv' exists after running Feature.py
    if not os.path.exists("generated_merged_num.csv"):
        print("Error: 'generated_merged_num.csv' was not generated by Feature.py.")
        sys.exit(1)
    ...
    if args.default_train_file:
        ...
        # Ensure proper standardization of columns in generated_merged_num.csv using HPBC_default_train_statistics.csv
        mean_std_file = "HPBC_default_train_statistics.csv"
        mean_std = pd.read_csv(mean_std_file, index_col=0)
        df_generated = pd.read_csv("generated_merged_num.csv")
        ...
        standardized_generated_file = "standardized_generated_merged_num.csv"
        ...
        # Calculate mean and std for generated_merged_num.csv
        df_generated = pd.read_csv("generated_merged_num.csv")
        ...
        df_standardized = (df_generated - mean_std.loc['mean']) / mean_std.loc['std']
        ...
        df_train[feature_set_1].to_csv(feature_set_1_file, index=False)
        report_file_status(feature_set_1_file, "User ML train feature set 1")

        feature_set_2_file = f"{model_name}_user_ML_train_feature_set_2.csv"
        df_train[feature_set_2].to_csv(feature_set_2_file, index=False)
        report_file_status(feature_set_2_file, "User ML train feature set 2")
        ...
        # Extract feature sets from standardized_user_ML_test
        df_test_standardized = pd.read_csv(standardized_test_file)
        test_feature_set_1_file = f"{model_name}_user_ML_test_feature_set_1.csv"
        df_test_standardized[feature_set_1].to_csv(test_feature_set_1_file, index=False)
        report_file_status(test_feature_set_1_file, "User ML test feature set 1")

        test_feature_set_2_file = f"{model_name}_user_ML_test_feature_set_2.csv"
        df_test_standardized[feature_set_2].to_csv(test_feature_set_2_file, index=False)
        report_file_status(test_feature_set_2_file, "User ML test feature set 2")
