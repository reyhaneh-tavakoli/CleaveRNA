...
def run_rnaplfold(target_file, LA, RA, output_dir, temperature):
    ...
    command = (
        f'RNAplfold -W 150 -L 100 -u {u_length} -T {temperature} < "{target_file_path}"'
    )
...
def process_specific_query(csv_file, output_dir):
    ...
    if not {"LA_seq", "RA_seq", "CS", "CS_Index_query", "Tem", "CA"}.issubset(
        df.columns
    ):
        raise ValueError(
            "The CSV file for specific_query mode must contain 'LA_seq', 'RA_seq', 'CS', 'CS_Index_query', 'Tem', and 'CA' columns."
        )
    ...
    for _, row in df.iterrows():
        ...
        with open(target_file, "r") as f:
            target_seq = convert_t_to_u(
                "".join([line.strip() for line in f if not line.startswith(">")])
            )

        # Validate the CS motif
        sequence_at_position = target_seq[
            start - 1 : end
        ]  # Adjust for 0-based indexing
        if sequence_at_position.upper() != CS.upper():
            print(
                f"Warning: CS motif '{CS}' does not match the sequence at position {start}-{end} in {target_file}. Found: '{sequence_at_position}'. Using the sequence from the target file."
            )
        ...
        query_file = os.path.join(
            output_dir,
            f"queries_{os.path.basename(target_file).split('.')[0]}_{start}_{end}.fasta",
        )
        with open(query_file, "w") as f:
...
def process_intarna_queries(
    ...
    for i, (name, seq) in enumerate(queries, 1):
        ...
        if len(parts) >= 3:
            start, end = (
                max(1, int(parts[1]) - 9),
                min(len(sequence), int(parts[2]) + 11),
            )
    ...
    for file in os.listdir():
        if (
            file.endswith("_Results_with_region.csv")
            or file.endswith("_Results_without_region.csv")
            or file.endswith("_Results_pairwise.csv")
        ):
            all_result_files.append(file)

    # Extract unique prefixes from the result files
    unique_prefixes = list(
        set(file.rsplit("_Results", 1)[0] for file in all_result_files)
    )
        ...
        for idx, file in enumerate(result_files, start=1):
            if os.path.exists(file):
                ...
                try:
                    ...
                    numerical_cols = numerical_cols.add_suffix(
                        f"_{output_prefix}_{idx}"
                    )
                    ...
                    merged_data = pd.concat(
                        [merged_data, numerical_cols], axis=0, ignore_index=True
                    )
    ...
    try:
        ...
        if not os.path.exists(pu_file):
            raise FileNotFoundError(
                f"✗ Unpaired probability file {pu_file} not found. Did RNAplfold run correctly?"
            )
        ...
        for i, result_file in result_files.items():
            ...
            if not os.path.exists(result_file):
                ...
                raise FileNotFoundError(
                    f"✗ Result file {result_file} not found. Did IntaRNA run correctly?"
                )
            ...
            if missing_cols:
                print(
                    f"✗ Missing required columns in {result_file}: {missing_cols}"
                )  # Added logging
                raise ValueError(
                    f"✗ Missing required columns in {result_file}: {missing_cols}"
                )
        ...
        if "id2" in merged_data.columns:
            ...
            def safe_get_min_pu(row, positions, direction):
                try:
                    ...
                    if direction == "u":
                        return min(
                            pu.loc[pos - p, "l1"] for p in positions if (pos - p) >= 0
                        )
                    else:
                        return min(
                            pu.loc[pos + p, "l1"]
        ...
        full_output_path = os.path.join(
            output_dir, f"{base_filename}_generated_merged.csv"
        )
...
def merge_all_generated_files(output_dir, final_output_file, targets_fasta_files=None):
    ...
    cols_to_remove = [
        ...
        if col in ["id2.1", "seq2.1", "target_file.1"]
        or col.startswith("id2.")
        or col.startswith("seq2.")
        or col.startswith("target_file.")
    ]
    if cols_to_remove:
        merged_data = merged_data.drop(columns=cols_to_remove)
    merged_data.to_csv(final_output_file, index=False)
    print(
        f"✓ Merged data saved to {final_output_file} (with only one set of id2, seq2, target_file columns)"
    )
...
def main(args=None):
    ...
    if args.prediction_mode == "specific_query":
        ...
        if missing_columns:
            raise ValueError(
                f"The following required columns are missing in the CSV file: {', '.join(missing_columns)}"
            )
        ...
        if not required_columns.issubset(params_df.columns):
            raise ValueError(
                "The CSV file must contain the columns: LA, RA, CS, Tem, and CA."
            )
    ...
    if "" in params_df.columns:
        raise ValueError(
            "The CSV file contains unnamed or empty column headers. Please fix the file."
        )
    ...
    if args.prediction_mode == "default":
        ...
        if not {"LA", "RA", "CS", "CS_index", "Tem", "CA"}.issubset(params_df.columns):
            raise ValueError(
                "The CSV file must contain the columns: LA, RA, CS, CS_index, Tem, and CA."
            )
        ...
        for _, row in params_df.iterrows():
            ...
            if len(motif) != 2:
                print(
                    f"Warning: Dinucleotide extraction failed at {dinuc_start}-{dinuc_end} in {target_file}. Extracted: '{motif}'. Skipping."
                )
                continue
            if (end - start + 1) != 2:
                print(
                    f"Warning: Provided region {start}-{end} is not a dinucleotide. Using dinucleotide at {dinuc_start}-{dinuc_end} instead."
                )
        ...
        for _, row in params_df.iterrows():
            ...
            if len(start_end_data) != 2:
                raise ValueError(
                    "Invalid format in Start_End_Index column. Expmected format: 'target_file:start-end' (e.g., '1.fasta:50-100')."
                )
            ...
            try:
                ...
                raise ValueError(
                    f"Invalid start-end positions in Start_End_Index column: {positions}. Expected format: 'start-end' (e.g., '50-100')."
                )
            ...
            for fasta_file in args.targets.split(","):
                ...
                if start < 0 or end > len(target_seq) or start >= end:
                    raise ValueError(
                        f"Invalid start-end positions {start}-{end} for file {target_file_name}. Must be within the bounds of the target sequence and start < end."
                    )

                print(
                    f"Checking target region from position {start} to {end} in the target sequence of {target_file_name}."
                )
                ...
                write_queries_to_fasta(queries, query_file)
                print(
                    f"Generated {len(queries)} queries for motifs in region {start}-{end} of {target_file_name}."
                )
        ...
        if not {"LA_seq", "RA_seq", "CS", "CS_Index_query", "Tem", "CA"}.issubset(
            ...
            raise ValueError(
                "The CSV file must contain the columns: LA_seq, RA_seq, CS, CS_Index_query, Tem, and CA."
            )

        processed_files = set()
        cs_dz_records = []  # To store id2, seq2, target_file for each query in order
        for _, row in params_df.iterrows():
            LA_seq, RA_seq, CS, temperature, core = (
                row["LA_seq"],
                row["RA_seq"],
                row["CS"],
                float(row["Tem"]),
        ...
        for fasta_file in fasta_files:
            ...
            if args.prediction_mode == "default":
                ...
                if "CS_index" not in row:
                    raise ValueError(
                        "For target_screen mode, the CSV file must contain a 'CS_index' column with the format 'target_file:CS_index' (e.g., '1.fasta:17')."
                    )

                cs_index_data = row["CS_index"].split(":")
                if len(cs_index_data) != 2:
                    raise ValueError(
                        "Invalid format in CS_index column. Expected format: 'target_file:CS_index' (e.g., '1.fasta:17')."
                    )
                ...
                try:
                    ...
                    raise ValueError(
                        f"Invalid CS_index format: {region}. Expected format: 'start-end' (e.g., '17-18')."
                    )
                ...
                if start < 0 or end > len(target_seq) or start >= end:
                    raise ValueError(
                        f"Invalid CS_index range {start}-{end} for file {target_file_name}. Must be within the bounds of the target sequence and start < end."
                    )
                ...
                if sequence_at_position.upper() != motif.upper():
                    print(
                        f"Warning: CS motif '{motif}' does not match the sequence at position {start} in {target_file_name}. Found: '{sequence_at_position}'. Using the sequence from the target file."
                    )
                ...
                if int(id2.split("-")[0]) != start:
                    raise ValueError(
                        f"CS_index {start} does not match the first number of id2 {id2.split('-')[0]}."
                    )
                ...
                if "Start_End_Index" not in row:
                    raise ValueError(
                        "For target_check mode, the CSV file must contain a 'Start_End_Index' column with the format 'target_file:start-end' (e.g., '1.fasta:50-100')."
                    )

                start_end_data = row["Start_End_Index"].split(":")
                if len(start_end_data) != 2:
                    raise ValueError(
                        "Invalid format in Start_End_Index column. Expected format: 'target_file:start-end' (e.g., '1.fasta:50-100')."
                    )
    ...
    merge_all_generated_files(
        ...
        targets_fasta_files=fasta_files,
    )
