...
def post_process_features(target_file, output_dir):
    ...
    try:
        ...
        # Define output_prefix based on the base_filename
        output_prefix = base_filename
        
        # Read unpaired probability data
        pu_file = f"{base_filename}_lunp"
        ...
        out = []
        result_files = {
            1: f"{output_prefix}_Results_with_region.csv",
            2: f"{output_prefix}_Results_without_region.csv",
            3: f"{output_prefix}_Results_pairwise.csv"
        }
